...
class EventDetailView(UserPassesTestMixin, DetailView):
    ...
    def set_template_name_suffix(self):
        if 0 <= self.object.status <= len(EventDetailView.template_suffix_from_status):
            self.template_name_suffix = self.template_suffix_from_status[
                self.object.status
            ]
    ...
    def get_proposals(self, context, lookup_vote_model, proposal_model):
        proposals = (
            proposal_model.objects.filter(user_event__event=self.object)
            .annotate(Count(lookup_vote_model))
            .order_by(f"-{lookup_vote_model}__count")
        )
        proposals = self.annotate_i_voted(
            proposals=proposals, lookup_vote_model=lookup_vote_model
        )
        return proposals
    ...
    def get_context_0(self, context):
        proposals = self.get_proposals(
            context=context,
            lookup_vote_model="dateproposalvote",
            proposal_model=DateProposal,
        )
        my_votes = self.get_votes(
            context=context, vote_model=DateProposalVote, proposals=proposals
        )
    ...
    def get_context_1(self, context):
        proposals = self.get_proposals(
            context=context,
            lookup_vote_model="placeproposalvote",
            proposal_model=PlaceProposal,
        )
        my_votes = self.get_votes(
            context=context, vote_model=PlaceProposalVote, proposals=proposals
        )
        context["my_votes"] = my_votes
        context["proposals"] = proposals
